...
-}
data Position = Start | OnBoard Square | Home deriving (Ord, Eq, Show)
{-
...
-}
type Placing = ([Position],[Position])            -- ! first part of tuple is for Red player, second part of tuple is for Green
placing :: Placing -> Position -> Player -> Int  -- ! Assuming placing must return the number of pieces of a player that are in a given Position (either Start, Onboard, or Home)
placing b p q = length [z == p | z <- (if q == Red then fst b else snd b)]

{- --? old code
placing :: Placing -> Position -> Player -> Int
placing b p q | q == Red = getPos (fst b) p
              | q == Green = getPos (snd b) p
              | otherwise = -1
getPos :: [Position] -> Position -> Int
getPos x pos = length [z == pos | z <- x]
-}
